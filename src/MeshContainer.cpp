...
void MeshContainer::setFilenames(const std::string& meshFilepath)
{
    ...
    meshFilename_ = path_ + "/" + fullPath.stem().string() + fullPath.extension().string();
    skinFilename_ = path_ + "/" + fullPath.stem().string() + "_surf" + fullPath.extension().string();
    boundFilename_ = path_ + "/" + fullPath.stem().string() + "_bound" + fullPath.extension().string();
    std::cout << boundFilename_ << std::endl;
    vacuumFilename_ = path_ + "/" + fullPath.stem().string() + "_vac" + fullPath.extension().string();
}

MeshPair& MeshContainer::userMesh() 
{
    return userMesh_;
}

MeshPair& MeshContainer::skinnedMesh() 
{
    return skinnedMesh_;
}

MeshPair& MeshContainer::boundaryMesh() 
{
    return boundaryMesh_;
}

MeshPair& MeshContainer::vacuumMesh()
{
    return vacuumMesh_;
}
