...
public:
   ...
   */
   SurfaceMeshGenerator(libMesh::Mesh& meshRef, libMesh::Mesh& surfaceMeshRef, std::vector<libMesh::Elem*> &elements = {}));
   ...
   /**
    * Method to retrieve the "skin" of an input \p mesh, and store it in \p
    * surfaceMesh. \p setting writeMesh to true will write the surfaceMesh out
    * with a given \p outputFilename, which by default is "surface_mesh.e".
    */
   //   void getSurface(libMesh::Mesh &mesh, libMesh::Mesh &surfaceMesh,
   //                   bool writeMesh = false,
   //                   std::string outputFilename = "surface_mesh.e");
