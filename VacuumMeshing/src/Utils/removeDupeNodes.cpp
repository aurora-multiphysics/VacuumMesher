...
void createTree(nodeTree &rtree, libMesh::Mesh &mesh_one, const double &tol) {
  // Add all existing verts to rTree
  for (auto &node : mesh_one.local_node_ptr_range()) {
...
void combineMeshes(const double &tol, libMesh::Mesh &mesh_one,
                   libMesh::Mesh &mesh_two,
                   std::multimap<unsigned int, unsigned int> surface_face_map) {
  std::set<libMesh::subdomain_id_type> sub_ids;
  mesh_one.subdomain_ids(sub_ids);
  ...
  for (auto &node : mesh_two.local_node_ptr_range()) {
    ...
    int node_offset = mesh_one.n_nodes();
    ...
    mesh_one.add_point(xyz, id_map[node->id()]);
  }
  ...
  // duplicate nodes
  for (auto &elem : libMesh::as_range(mesh_two.local_elements_begin(),
                                      mesh_two.local_elements_end())) {
    unsigned int el_id = 1 + mesh_one.n_elem();
    // Element type being build must be consistent between meshes
    libMesh::Elem *new_elem =
        libMesh::Elem::build(mesh_one.elem_ptr(0)->type()).release();
    for (int j = 0; j < new_elem->n_nodes(); j++) {
      new_elem->set_node(j) =
          mesh_one.node_ptr(id_map[elem->node_ptr(j)->id()]);
    }
    new_elem->set_id(el_id);
    new_elem->subdomain_id() = vac_id;
    mesh_one.add_elem(new_elem);
...
// Version of the method that doesn't require a surface_face_map, just combines
// the meshes
void combineMeshes(const double &tol, libMesh::Mesh &mesh_one,
                   libMesh::Mesh &mesh_two) {
  // Instantiate an rTree. Using a rTree data structure significantly reduces
