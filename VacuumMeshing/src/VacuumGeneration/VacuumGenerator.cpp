...
VacuumGenerator::VacuumGenerator(libMesh::Mesh &mesh, libMesh::Mesh &surface_mesh, 
                                 libMesh::Mesh &boundary_mesh, libMesh::Mesh &vacuum_mesh, 
                                 std::multimap<unsigned int, unsigned int> *surface_face_map)
    : mesh_(mesh), surface_mesh_(surface_mesh), boundary_mesh_(boundary_mesh), vacuum_mesh_(vacuum_mesh){
      
      if(surface_face_map != nullptr)
      {
        surface_face_map_ = std::make_unique<std::multimap<unsigned int, unsigned int>>(*surface_face_map);
      }
...
void VacuumGenerator::generateVacuumMesh(const std::string tet_settings)
{
  ...
  //  They are needed so that the tetrahedralisation methods know where to not generate tets 
...
void VacuumGenerator::tetrahedraliseVacuumRegion(const Eigen::MatrixXd &seed_points,
...
void VacuumGenerator::tetrahedraliseVacuumRegion(const Eigen::MatrixXd &V,
...
Eigen::MatrixXd VacuumGenerator::getSeeds(const libMesh::Mesh mesh) const{
  ...
  for (int i; i < N_faces.rows(); i++) {
    ...
    for (auto &vert : verts) {
      seed[0] += (V.row(vert)[0] - seeding_tolerance_ * normal[0]) / 3; // x component of seed
      seed[1] += (V.row(vert)[1] - seeding_tolerance_ * normal[1]) / 3; // y component of seed
      seed[2] += (V.row(vert)[2] - seeding_tolerance_ * normal[2]) / 3; // z component of seed
    }
